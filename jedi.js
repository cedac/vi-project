...
d3.json("dataset.json", function (data) {
    ...
    for (i in keys) {
        for (key2 in item[keys[i]]) {
            array.push({
                metric: keys[i],
                year: +key2,
                value: +item[keys[i]][key2],
                place: +i
            });
        } 
    }
...
function genHeatMap() {
    ...
    var x = d3.scaleLinear().range([0, width]),
        ...
        z = d3.scaleLinear().range(["green", "red"]);
  ...
  svg.append("g")
        ...
        .call(d3.svg.axis().scale(x).ticks(d3.time.years).tickFormat(formatDate).orient("bottom"))
        .append("text")
        .attr("class", "label")
        .attr("x", width)
        .attr("y", -6)
